...
import { EntityArrayResponseType, SpotifyAccountService } from '../service/spotify-account.service';
...
export class SpotifyAccountComponent implements OnInit {
  ...
  trackId = (_index: number, item: ISpotifyAccount): number => this.spotifyAccountService.getSpotifyAccountIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.spotifyAccounts = this.refineData(dataFromBody);
  }

  protected refineData(data: ISpotifyAccount[]): ISpotifyAccount[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: ISpotifyAccount[] | null): ISpotifyAccount[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.spotifyAccountService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
